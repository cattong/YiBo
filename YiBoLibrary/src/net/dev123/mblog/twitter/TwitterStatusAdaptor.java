	...
	public static List<Status> createSearchResultList(String jsonString) throws LibException {
		try {
			...
			for (int i = 0; i < size; i++) {
				...
				user = new User();
				user.setId(getRawString("from_user_id_str", json));
				...
				status = new Status();
				status.setId(getRawString("id_str", json));
				...
				if (json.has("entities")) {
					...
					if (entiiesJson.has("media")) {
						...
						status.setThumbnailPicture(imageUrl + ":thumb");
						status.setOriginalPicture(imageUrl + ":large");
					}
				}
