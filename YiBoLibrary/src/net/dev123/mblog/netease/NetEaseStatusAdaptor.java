...
public class NetEaseStatusAdaptor {
	...
	static Status createStatus(JSONObject json) throws LibException {
		try {
			...
			if (StringUtil.isNotEmpty(status.getInReplyToStatusId())) {
				 ...
				 * 这里取原始微博（root开头的字段）。
				 */
				retweet.setId(getRawString("root_in_reply_to_status_id", json));
				 ...
				 */
				user.setId(getRawString("root_in_reply_to_user_id", json));
	...
	public static void extractImageUrl(Status status) {
		...
		while (matcher.find()) {
			...
			status.setThumbnailPicture(imageUrl);
			status.setMiddlePicture(imageUrl);
			status.setOriginalPicture(imageUrl);
		}
		...
		try {
			...
			if (!jsonObject.isNull("results")) {
				...
				for (int i = 0; i < size; i++) {
					...
					User user = new User();
					user.setServiceProvider(ServiceProvider.NetEase);
					user.setId(getRawString("from_user_id",json));
					...
					if (!json.isNull("to_status_id")) {
						...
						retweet.setServiceProvider(ServiceProvider.NetEase);
						retweet.setId(getRawString("to_status_id", json));
						retweet.setText(Emotions.normalizeEmotion(
								ServiceProvider.NetEase, getRawString("to_status_text", json)));

						User rtUser = new User();
						rtUser.setServiceProvider(ServiceProvider.NetEase);
						rtUser.setId(getRawString("to_user_id", json));
