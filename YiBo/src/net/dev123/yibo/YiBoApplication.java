...
import net.dev123.yibo.service.task.YiBoMeUrlDetectTask;
...
public class YiBoApplication extends Application {
    ...
    private void initCachePath() {
		...
		String oldSdcardCachePath = sdcardPath.getAbsolutePath() + File.separator + "yibo";
	...
	private BroadcastReceiver globalReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (GlobalVars.NET_TYPE != NetType.NONE) {
				if (looper == 0) {
					new YiBoMeUrlDetectTask().execute();
				}
				looper = (++looper >= 5 ? 0 : looper);
			}
		}
	};
