...
import net.dev123.yibo.AccountsActivity.Action;
...
public class AccountListAdapter extends ArrayAdapter<LocalAccount> {
	...
	private void initList() {
		...
		sql.append("select * from Account where State != ").append(Account.STATE_DELETED);
		if (newAccountId > 0) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		String spName = account.getServiceProvider().getServiceProviderName();
		if (account.isDefault()) {
		...
		if (account.getTokenExpiresAt() != null 
			&& account.getTokenExpiresAt().getTime() < System.currentTimeMillis()) {
			...
			public void onClick(View v) {
				...
				.setPositiveButton(R.string.btn_confirm,
					new AlertDialog.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							try {
								account.setState(Account.STATE_DELETED);
	...
	public synchronized void editAccount(Action action, LocalAccount account) {
		...
		case Delete:
			...
			if (yibo.getCurrentAccount()!= null
				&& yibo.getCurrentAccount().getAccountId().equals(account.getAccountId())
				&& getCount() > 0 ) {
				//如果删除的帐号是当前帐号，且删除后帐号列表中帐户数多于一个
				yibo.setCurrentAccount(getItem(0));
			}
